...
func RegisterScratchTools(mcpServer *mcp.Server, resourceMap map[string]config.ResourceItem, tmpDir string, verbose bool) {
	mcp.AddTool(mcpServer,
		&mcp.Tool{Name: "CreateFile", Description: "Creates a new file in the scratch space."},
		func(ctx context.Context, request *mcp.CallToolRequest, req CreateFileRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
				log.Printf("Handling CreateFile request for path: %s", req.Path)
			}
			res, err := createFile(tmpDir, req.Path, req.Content)
			return res, shared.EmptyOutput{}, err
		})

	mcp.AddTool(mcpServer,
		&mcp.Tool{Name: "ReadFile", Description: "Reads the content of a file in the scratch space."},
		func(ctx context.Context, request *mcp.CallToolRequest, req ReadFileRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
	...
	mcp.AddTool(mcpServer,
		...
		func(ctx context.Context, request *mcp.CallToolRequest, req DeleteFileRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
	...
	mcp.AddTool(mcpServer,
		...
		func(ctx context.Context, request *mcp.CallToolRequest, req ReplaceInFileRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
	...
	mcp.AddTool(mcpServer,
		...
		func(ctx context.Context, request *mcp.CallToolRequest, req ListDirectoryRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
	...
	mcp.AddTool(mcpServer,
		...
		func(ctx context.Context, request *mcp.CallToolRequest, req CreateDirectoryRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
	...
	mcp.AddTool(mcpServer,
		...
		func(ctx context.Context, request *mcp.CallToolRequest, req RemoveDirectoryRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
	...
	mcp.AddTool(mcpServer,
		...
		func(ctx context.Context, request *mcp.CallToolRequest, req CopyResourceToFileRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
	...
	mcp.AddTool(mcpServer,
		...
		func(ctx context.Context, request *mcp.CallToolRequest, req CopyResourceTreeRequest) (*mcp.CallToolResult, shared.EmptyOutput, error) {
			if verbose {
...
func copyResourceToFile(resourceMap map[string]config.ResourceItem, tmpDir string, verbose bool, resourceURI, path string) (*mcp.CallToolResult, error) {
	item, ok := resourceMap[resourceURI]
	if !ok {
		return shared.NewErrorResult(fmt.Sprintf("resource not found: %s", resourceURI)), nil
	}

	content, err := resource.GetResourceContent(item, tmpDir, verbose)
	if err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to get resource content for %s: %v", resourceURI, err)), nil
	}

	if content == "" {
		return shared.NewErrorResult(fmt.Sprintf("resource %s has no content or command", resourceURI)), nil
	}

	return createFile(tmpDir, path, content)
}

func copyResourceTree(resourceMap map[string]config.ResourceItem, tmpDir string, verbose bool, resourcePrefix, destinationPath string) (*mcp.CallToolResult, error) {
	...
	if len(matchedURIs) == 0 {
		return shared.NewErrorResult(fmt.Sprintf("no resources found matching prefix: %s", resourcePrefix)), nil
	}
	...
	for _, uri := range matchedURIs {
		...
		content, err := resource.GetResourceContent(item, tmpDir, verbose)
		if err != nil {
			return shared.NewErrorResult(fmt.Sprintf("failed to get resource content for %s: %v", uri, err)), nil
		}

		if content == "" {
			return shared.NewErrorResult(fmt.Sprintf("resource %s has no content or command", uri)), nil
		}
	...
	return shared.NewTextResult(fmt.Sprintf("Successfully copied %d resources to %s.", len(matchedURIs), destinationPath)), nil
}


func createFile(tmpDir, path, content string) (*mcp.CallToolResult, error) {
	fullPath, err := shared.ResolvePath(tmpDir, path)
	if err != nil {
		return shared.NewErrorResult(err.Error()), nil
	}
	dir := filepath.Dir(fullPath)
	if err := os.MkdirAll(dir, 0755); err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to create parent directories: %v", err)), nil
	}
	if err := os.WriteFile(fullPath, []byte(content), 0644); err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to create file: %v", err)), nil
	}
	return shared.NewTextResult("File created successfully."), nil
}
...
func readFile(tmpDir, path string) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to read file: %v", err)), nil
	}
	return shared.NewTextResult(string(content)), nil
}
...
func deleteFile(tmpDir, path string) (*mcp.CallToolResult, error) {
	...
	if err := os.Remove(fullPath); err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to delete file: %v", err)), nil
	}
	return shared.NewTextResult("File deleted successfully."), nil
}
...
func replaceInFile(tmpDir, path, pattern, replacement string, replaceAll bool) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return shared.NewErrorResult(fmt.Sprintf("invalid regular expression: %v", err)), nil
	}

	var newContent string
	if replaceAll {
		if !re.MatchString(content) {
			return shared.NewErrorResult("pattern not found in file"), nil
		}
	...
	if err := os.WriteFile(fullPath, []byte(newContent), 0644); err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to write modified file: %v", err)), nil
	}
	return shared.NewTextResult("File modified successfully."), nil
}
...
func listDirectory(tmpDir, path string) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to list directory: %v", err)), nil
	}
	...
	return shared.NewTextResult(out.String()), nil
}
...
func createDirectory(tmpDir, path string) (*mcp.CallToolResult, error) {
	...
	if err := os.MkdirAll(fullPath, 0755); err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to create directory: %v", err)), nil
	}
	return shared.NewTextResult("Directory created successfully."), nil
}
...
func removeDirectory(tmpDir, path string) (*mcp.CallToolResult, error) {
	...
	if err := os.Remove(fullPath); err != nil {
		return shared.NewErrorResult(fmt.Sprintf("failed to remove directory: %v", err)), nil
	}
	return shared.NewTextResult("Directory removed successfully."), nil
}
